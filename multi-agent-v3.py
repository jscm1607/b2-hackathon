...
def create_multi_agent_ui():
    ...
    tabs = st.tabs(["Data Loading", "Historical Context", "Collaborative Analysis", "Agent Communication", "Data Visualization"])
    ...
    create_visualization_ui(tabs[4])
...
def create_visualization_ui(tab):
    with tab:
        st.subheader("Biosphere 2 Data Visualization Tool")
        st.write("Upload your CSV files to visualize the data")
        
        # File uploader
        uploaded_files = st.file_uploader("Choose CSV files", type="csv", accept_multiple_files=True, key="viz_files")
